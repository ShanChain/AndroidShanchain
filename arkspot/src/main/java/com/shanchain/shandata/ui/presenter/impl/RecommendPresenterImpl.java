...
public class RecommendPresenterImpl implements RecommendPresenter {
    ...
    private void obtainSpaceInfo( final boolean isLast) {

        List<Integer> spaceIds = new ArrayList<>();
        for (int i = 0; i < datas.size(); i++) {
            StoryModel storyModel = datas.get(i);
            int spaceId = storyModel.getModelInfo().getBean().getSpaceId();
            spaceIds.add(spaceId);
        }

        String jArr = JSONObject.toJSONString(spaceIds);
